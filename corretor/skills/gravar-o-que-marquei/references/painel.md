...
O corretor **decide quando quiser**, e não só quando você pergunta: na
página inicial, o cliente que está numa etapa do funil tem dois botões —
...
Quem a grava SEM outro trabalho junto é `/corretor:gravar-o-que-marquei` — é a
...
**Declare o `gesto` nas `decisoes`** de toda `lista` que julga clientes:
...
cliente e o botão equivale a mudá-lo de etapa:
...
**Completar informações**: chama `/corretor:completar-ficha` com o id. Quando
...
cliente; `resumo`, títulos de seção; em `rotulos` e `proximo`, as skills
...
Na página de um cliente, no cartão do início e na linha da tabela, o painel
mostra **o que vem agora**: um botão em destaque e os outros ao lado. A ordem
por etapa é do pack. Mas **você pode apontar o destaque de UM cliente**, e o
lugar é a linha dele no `funil.md`, que você já escreve:

```
- V-071 (casa 3 dorm, Azenha) · desde 2026-09-08 · próximo: cobrar-o-que-falta — o documento que ficou de mandar
```

Quando o que vem depois de `próximo:` COMEÇA pelo nome de uma skill do pack —
`cobrar-o-que-falta`, ou `/corretor:cobrar-o-que-falta` —, ela vira o destaque daquele
cliente, e o que vem depois do travessão é a nota que a pessoa lê debaixo do
...
`← corretor, no painel, AAAA-MM-DD`. `descartar` é a Regra 3 — aposentar,
...
para a carteira pelas regras de sempre, com a origem `← corretor, no
painel`.
