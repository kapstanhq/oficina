...
description: >-
  Grava na carteira as decisões que o corretor marcou no painel sem estar
  numa conversa — cliente que passa para a etapa seguinte do funil e
  cliente que sai, com o motivo. Lê a fila do painel, grava cada decisão pelas
  ...
  o corretor disser — grava o que eu marquei no painel · grava as minhas
  ...
  Não é para a lista do dia, que é /corretor:o-que-fazer-hoje, nem para arrumar
  a carteira inteira, que é /corretor:organizar-carteira.
license: MIT
compatibility: >-
  ...
  linha e não grava nada. Sem carteira, manda rodar /corretor:comecar.
...
Na página inicial do painel o corretor decide quando quiser: o cliente
...
4.4  o arquivo do imóvel             4.5  o arquivo do cliente
...
As decisões já são do corretor: ele as tomou no painel, com o botão. Por
...
do painel já avisou o corretor.

### Passo 3 · As passagens de etapa

Para cada `etapa`, no arquivo do cliente: a linha `etapa:` passa a `para`,
com a procedência `← corretor, no painel, AAAA-MM-DD`, e o `## Histórico`
...
arquivo, o arquivo para `arquivo-morto/clientes/`, a linha do
...
o arquivo do cliente já é o dono do fato.

### Passo 5b · As respostas que chegaram atrasadas

`painel_inicio` pode trazer `respostas`: telas que o corretor respondeu
...
## Guardei
- ~/carteira/clientes/<arquivo> — a etapa e o histórico
- ~/carteira/arquivo-morto/clientes/<arquivo> — aposentado
...
decidido, e não um texto para o cliente (contrato §10).
