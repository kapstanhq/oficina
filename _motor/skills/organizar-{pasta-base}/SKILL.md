...
compatibility: >-
  ...
  {base} e não funciona sem uma — diz isso em uma linha e manda rodar
...
que só aparece olhando a {base} inteira de cima — e **relata cada arquivo que
tocou, com o caminho**, porque o {profissional} nunca pode abrir a {base} e não
achar o que tinha.

**O argumento é opcional:** sem nada, ela varre a {base} inteira; com o id de
{um-pessoa} ou de {um-item}, ela trabalha só esse item.

**Quem só OLHA é outra skill.** `/{plugin}:laudo-da-{pasta-base}` lê a {base} e
...
arquivo que não existe.** Sobrescrever um arquivo da {base} apaga o que outra
...
1. **O contrato, por seção.** Ele é a lei do pack, e ler o documento inteiro
   custa vinte a trinta mil tokens antes da primeira leitura da {base}.
   ...
   `{secao-arquivo-pessoa}`, `04-6-os-dois-indices.md` e `04-7-o-bruto.md` (os
   formatos literais — `{secao-arquivo-item}` é o gabarito da ficha que a
   ...
   `11-0-onde-roda.md` (onde ela roda, e por que não funciona sem {base}).
   Nada do que está lá se reescreve aqui: divergiu, o contrato vence.

2. **o `INDICE.md` da {base}**, pela primeira leitura da seção 1 do contrato:
   procura no computador e, não achando, a pasta `{pasta-base}` no Drive. A linha
   `{pasta-base}:` dele diz o transporte — `local` ou `drive` —, e **toda leitura e
...
**A {base} não existe** — não há `INDICE.md` em transporte nenhum? Uma linha,
e só:

> Não achei a sua {base}, nem no computador nem no seu Drive. Rode
> `/{plugin}:comecar`: ele monta em alguns minutos e termina com {um-item} e um
> {pessoa} de verdade dentro dela.

Não crie a {base}, não trabalhe sem ela, não improvise em outra pasta. Esta
...
parcial. Sem argumento, a {base} inteira.
...
- **Não aposenta por outro motivo que não o prazo de {prazo-de-silencio}.** {motivos-fora-do-prazo}{outro-motivo-que-e-dele}: é
...
não só quando a {base} é grande. Demorar sem mostrar é onde o leigo acha que
travou.

```
1  ler a {base} e listar o que precisa de atenção
...
```
~/{pasta-base}/{pasta-itens}/*.md
~/{pasta-base}/{pasta-pessoas}/*.md
~/{pasta-base}/_bruto/*
~/{pasta-base}/arquivo-morto/{pasta-itens}/*.md
~/{pasta-base}/arquivo-morto/{pasta-pessoas}/*.md
```
...
leitura seja barata**: vinte {pasta-pessoas} de sessenta linhas cabem numa passada. Se a {base}
...
procure   o nome do bruto: {exemplo-bruto}
          em: ~/{pasta-base}/{pasta-itens}/  ~/{pasta-base}/{pasta-pessoas}/  ~/{pasta-base}/arquivo-morto/
              e os outros donos: os arquivos da raiz da {base} e as pastas que
...
Um `Grep` por bruto são centenas de chamadas: numa {base} de 180 brutos, a
...
arquivo da raiz ou de outra pasta da {base} é citado lá, e procurá-lo só em
`{pasta-itens}/` e `{pasta-pessoas}/` o dava como não lido em toda execução.
...
é /{plugin}:<skill>` —, ele não é trabalho desta: vira uma linha só em
...
[[se etapa-de:item]]
- **A etapa é {do-item}, e a conversa é com {o-pessoa}.** O fato de quem
  falou vai para o arquivo de quem falou; o que a conversa mostra de andamento
  vai para o `etapa:` {do-item} que nomeia essa pessoa em
  `{campo-da-pessoa-no-item}`. O arquivo {do-pessoa} **não tem `etapa:`**, nem o
...
[[fim]]
- **{Item} que não está na {base}: não crie.** Sem link e sem ficha {o-item}
  não entra (contrato, seção 7) — dado que se adivinhou vira {exemplo-de-erro-na-mensagem} no
...
[[se etapa-de:item]]
   ...
   4.3: {estado-da-planilha}, e o
   que não casar entra em `{etapa-inicial}`, nunca `?`.
[[fim]]
2. **Linha que já está na {base} atualiza, não duplica.** O `link:` ou o
...
4. **Depois das fichas:** uma linha por {item} em `{pasta-itens}/_indice.md`, o
   ...
   `## Histórico`: `- AAAA-MM-DD entrou na {base}  ← _bruto/AAAA-MM-DD-planilha-<nome>.csv`.

**Teto: 200 linhas por execução.** Passou, pergunte se importa tudo ou só as
{filtro-da-planilha}. Na tela vai **a primeira ficha inteira** e a
...
[[fim]]
[[se etapa-de:item]]
| {item} sem movimento há mais de {prazo-de-silencio} | **a skill** — nos dois modos |
[[fim]]
...
[[fim]]
[[se etapa-de:item]]
Os {prazo-de-silencio} contam da data mais nova entre o `## Histórico` e o
`## Combinado` do arquivo {do-item} — e, se `{campo-da-pessoa-no-item}` nomeia alguém, o
`## Histórico` dessa pessoa entra na conta: resposta que chegou por ela é
movimento {do-item}. **Nenhuma data em lugar nenhum: não aposenta por dedução**
— vira uma linha no relatório dizendo que {o-item} está sem data de movimento
desde que entrou.

**Quem sai é {o-item}, e só.** {O-pessoa} que `{campo-da-pessoa-no-item}` nomeia
continua onde está: pode responder por {outro-item}, e gente só sai da {base} por
pedido de silêncio ou por decisão do {profissional}. {Item} que nunca teve
{pessoa} se aposenta pela mesma régua — não ter com quem falar é o caso comum,
...
{base} fica exatamente como estava. {Base} com o {mesmo-andante} em duas
pastas é pior que {base} por organizar.

### Passo 5 · Os tetos

```
INDICE.md              120 linhas
arquivo de {pessoa}      {teto-pessoa} linhas
arquivo de {item}       {teto-item} linhas
...
E no arquivo {do-pessoa}: `- histórico até 2026-05-19 em _bruto/{exemplo-bruto-de-poda}`.
...
qual fica — id duplicado é a única coisa que quebra a {base} inteira, porque
...
|---|---|
| {andante} no `funil.md` sem arquivo em `{pasta-andantes}/` | pergunta: criar a ficha com `?` ou tirar a linha. No automático, cria |
| arquivo de {andante} com `etapa:` e sem linha no `funil.md` | reescreve o funil — a vista é derivada, o arquivo é dono |
...
Divergiu um fato apurado de outro fato apurado — {exemplo-de-fatos-que-divergem}? **Ela mantém os dois e relata os dois com a
...
```
1  id duplicado no _indice.md            quebra a {base}
...
**Quando não perguntar:** o fato está na {base} (use e cite de onde veio); é
...
origem, vírgula, data — {exemplo-linha-com-procedencia}. As origens são
...
sem cópia. O que sai de vista continua na {base}, e o relatório diz onde —
...
pastas — é pior que a {base} do jeito que estava.

**Ela não abre link.** As ferramentas dela leem a {base}, não a internet, e é
de propósito: varrer uma {base} abrindo trinta páginas é uma execução de meia
hora que termina com o {profissional} sem saber o que mudou. O `?` que um link
resolveria sai em `## Falta saber` com o nome de quem abre: `/{plugin}:{skill-que-abre-link}`
...
o {exemplo-trabalho} é de `/{plugin}:{skill-anunciar}`. Esta aqui arruma a {base} de onde
...
**Ela não sai da {base}.** Não roda git, não sincroniza, não faz backup,
não toca em pasta nenhuma fora dali, e não sabe o que a {sistema-do-oficio} tem no CRM.

**Ela para no décimo bruto** e diz quantos ficaram. E se a {base} estiver tão
