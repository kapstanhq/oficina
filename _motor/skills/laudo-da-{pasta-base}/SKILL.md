---
name: laudo-da-{pasta-base}
description: >-
  A régua barata da {base}: lê tudo e não escreve nada. Diz o que está sem
  procedência, o que está com procedência VENCIDA, que {item} ou {pessoa} ficou
  ...
  falta de outra pessoa é /{plugin}:cobrar-o-que-falta.
license: MIT
compatibility: >-
  Precisa da {base} e só de leitura — não escreve, não move e não apaga, em
  nenhum modo. Sem {base} ela diz isso em uma linha e manda rodar
  /{plugin}:comecar. Funciona igual no computador e no Drive; no Drive ela lê
...
Ela **lê a {base} inteira e não escreve uma linha**. O que ela devolve é um
...
horas importam pela mesma razão: uma {base} pouco aberta desatualiza, e
{base} desatualizada faz toda skill mentir com confiança. O `?` honesto vira
frase afirmativa, {exemplo-do-dado-que-envelheceu}, e o {profissional}
descobre no pior lugar possível — na frente {do-pessoa}.

**Ela não conserta nada.** Não move arquivo, não apaga, não reescreve índice,
não aposenta ninguém. Quem faz isso é `/{plugin}:organizar-{pasta-base}`, e a
...
1. Leia `~/{pasta-base}/INDICE.md`. Não existe: uma linha dizendo que não há
   {base} para conferir, e `/{plugin}:comecar`. Não invente {base} vazia.
...
5. Monte o inventário sem abrir tudo: os dois `_indice.md`, o `funil.md`, o
   `hoje.md` e a lista de nomes de `{pasta-itens}/`, `{pasta-pessoas}/` e
...
A ordem é de **consequência**, e não de pasta: o que faz o {profissional} dizer
uma coisa errada hoje vem antes do que deixa a {base} feia.

### 4.1 · Que campo está sem procedência

A regra 2 do contrato: todo campo leva de onde veio. Campo preenchido **sem** a
marca de origem é o achado mais grave da {base}, porque ele é indistinguível
de um campo apurado — e é exatamente o que a próxima skill vai afirmar para
alguém de fora.

Procure, em `{pasta-itens}/` e `{pasta-pessoas}/`, a linha de campo que tem
...
vence; fato que depende do {profissional} não vence.** O nome de quem decide não
envelhece; {exemplos-do-que-vence}, sim.

O achado é `arquivo · campo · apurado em <data> · há N dias`, e o conserto é
sempre perguntar de novo — nunca deduzir.

### 4.3 · Que {item} ou {pessoa} ficou órfão
...
- **id sem apelido** — o contrato pede `{exemplo-item}`, nunca o id sozinho.
  Onde o apelido faltar, o {profissional} não sabe do que se está falando.
[[se etapa-de:item]]

E uma quinta, que só existe porque aqui a etapa é {do-item}:

- **etapa sem dono** — `etapa:` num arquivo {do-pessoa}. O `funil.md` sai dos
  arquivos {do-item}, e aquele campo ninguém lê: ele existe e não vale. O
  achado nomeia o arquivo; quem decide o que fazer com a linha é o {profissional}.

**O que NÃO é órfão:** {item} com `etapa:` e `{campo-da-pessoa-no-item}` vazio.
{Item} sem {pessoa} é o caso comum, e listá-lo enche o laudo de linha que
ninguém vai consertar. Órfão é o contrário — o campo nomeia um id, e o id não
tem arquivo em `{pasta-pessoas}/`: é a primeira forma, e entra como ela.
[[fim]]
...
O `?` é a lista de trabalho da {base}, e ela envelhece em silêncio. Ordene por
...
Um `?` que espera há mais tempo que o {prazo-de-silencio} não é mais uma
pergunta em aberto: é uma decisão de não perguntar. Ele aparece no laudo com
essa palavra, para o {profissional} decidir se ainda quer a resposta.
...
Se a {base} estiver ilegível — pasta sem permissão, arquivo corrompido, Drive
...
coisa, e é o ponto: quem lê precisa saber que a {base} **não mudou**.
...
dele. Não é a lista dos `?` da {base} — essa é a seção 4.5, e ela é o
trabalho, não a confissão.

**Zero achados é um resultado, e ele se escreve:** `**A {base} está limpa:
nada sem procedência, nada vencido, nada órfão, nada acima do teto.**` Um laudo
que some quando está tudo certo ensina o {profissional} a não rodá-lo.
...
**Ela não abre link e não sai da {base}.** O `?` que um link resolveria é
achado, não trabalho — varrer a {base} abrindo trinta páginas transforma uma
...
pergunta, ela entra em `_motor/skills/laudo-da-{pasta-base}/SKILL.md` e passa a
