...
description: >-
  Procura o que falta em {um-item} da {base} — os campos do cabeçalho que estão
  em `?` — nas fontes que a {base} alcança: a origem guardada em `_bruto/`, o
  ...
  {profissional} disser — completa essa ficha · procura o que falta · o que ela
  ...
  chamar. Não é para julgar se serve, que é /{plugin}:{skill-de-julgar}, nem
  para cobrar alguém, que é /{plugin}:cobrar-o-que-falta.
license: MIT
compatibility: >-
  Precisa da {base} e do id de um registro dela. O que ela alcança depende do
...
ele entrou. Alguns decidem se ele serve ao {profissional}, e ele abre a ficha,
...
{profissional} confirmar — não é fato, e não se completa aqui. Se ele e as
seções de análise estão vazios, o fecho diz que o próximo passo é
`/{plugin}:{skill-de-julgar}`.
...
4.4  o arquivo {do-item}             4.7  o bruto
4.6  os dois índices                 10   como uma skill começa e termina
```

Os arquivos: `02-0-id-e-apelido.md`, `03-0-as-tres-regras.md`,
`{secao-arquivo-item}`, `04-6-os-dois-indices.md`, `04-7-o-bruto.md` e `10-0-comeca-e-termina.md`. E
...
dono dele. Não chegou: pergunte qual, em uma linha. Não percorra a {base}
...
- se a fonte diz que o registro **saiu do ar** ou fechou, isso é fato: o campo
  ...
  `/{plugin}:organizar-{pasta-base}` — ela não aposenta
...
- a linha do `funil.md` e o `_indice.md` que repetem um campo que mudou mudam
  juntos (Regra 1: o arquivo {do-item} é o dono, as vistas acompanham)
...
## Guardei
- ~/{pasta-base}/<arquivo {do-item}> — os campos e o histórico
- ~/{pasta-base}/_bruto/<arquivo> — a página que li

## Falta saber
- <o campo que só o {profissional} ou a organização respondem, e a quem perguntar>
````
...
linha no fecho, apontando `/{plugin}:{skill-de-julgar}`.
...
{profissional} conferir cada fonte antes de o campo virar verdade na {base}.
