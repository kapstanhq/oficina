...
description: >-
  ...
  texto e o destinatário na tela antes. Use quando o {profissional} disser — o
  ...
  /{plugin}:retomar-contato, nem para a lista do dia, que é
...
Isso é o oposto do resto do pack. `/{plugin}:o-que-fazer-hoje` lista o que o
{profissional} tem que fazer; esta lista o que ele **não pode fazer** porque está
...
- **Não é retomada.** Quem sumiu do funil sem dever nada é
  `/{plugin}:retomar-contato`. Aqui a pessoa **deve uma coisa concreta**, e a
...
coisa**. O {profissional} escolhe quem recebe.
...
2. **Os `?` com dono** nos arquivos de `{pasta-itens}/` e `{pasta-pessoas}/`. Um
...
Uma pendência é `{exemplo-pendencia}` — quem, o quê, desde quando.
...
- **Cobrado duas vezes sem resposta.** Passa a ser conversa do {profissional},
...
A ordem se explica em uma linha por item — o {profissional} precisa saber por que
...
{profissional} copia e cola. Nada de moldura desenhada.
...
## Fora da fila
- {exemplo-pessoa} · cobrado em 2026-09-07, há 2 dias — espere até o dia 14
...
para um número novo sai do celular do {profissional}. Aqui isso quase nunca
