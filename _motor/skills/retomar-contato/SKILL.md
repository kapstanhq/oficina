...
description: >-
  ...
  {profissional} diz “quem sumiu?”, “quem parou de responder”, “preciso dar um toque em alguém”, “semana fraca, quem eu
  ...
  (/{plugin}:cobrar-o-que-falta), nem para responder quem respondeu
...
arquivo {do-item} nomeia. Chamar com {um-item} também vale — é o que o botão do
painel faz na página {do-item}, com o id —, e aí ela olha só {esse-item}: o
passo 1 roda sobre ele, e se ele não está parado, ou não tem {pessoa}, a saída
...
{profissional} aprovar, uma mensagem por vez. O bloco não some em nenhum dos dois
...
é o começo do nome do arquivo: `04-5-arquivo-de-{pessoa}.md` é a 4.5. Ele é o
padrão comum das {n-skills} skills do pack, e nada de formato se decide aqui; o
...
a voz das mensagens), o `canal padrão com {pessoa}:`, o
`{campo-horario-de-encontro}:` — este é o que fecha a mensagem sem
...
responder sem perguntar   não muda nada aqui — retomada não é resposta a
                          conversa viva, é o {profissional} começando de novo
...
`/{plugin}:{skill-conferir}`. Se em algum ponto parecer que ela precisa de
...
dela não tem {campos-do-filtro-nem}, então não dá para saber o que é novidade
para ela”.
...
acima disso o {profissional} rola, para de ler o texto de cada um e aprova no
atacado — que é exatamente o que esta skill existe para não fazer. {Profissional} com
...
arquivo {do-pessoa}. O que é próprio da retomada:
...
{profissional} aprovar. Cinco por execução cabem no teto de partida de seis conversas
...
<a tela, e a escolha do {profissional}>
...
é o que faz o {profissional} parar de confiar na lista.
...
três aprovadas juntas  três mensagens DIFERENTES, uma por pessoa, com o nome
                       dela, {o-item} dela e a novidade dela dentro. A ponte
                       manda uma, espera, manda a outra
```

O que o {profissional} aprova de uma vez é a **revisão**, não o disparo. Diga isso na
...
o mesmo ângulo, não    o mesmo assunto com outras palavras é a mesma mensagem,
                       e {o-pessoa} lê como a mesma mensagem
```
...
Retomada é o {profissional} **começando** a conversa: o destinatário não escreveu
...
`/{plugin}:comecar`. Aqui é lembrete, e lembrete que vira sermão é o que faz o
{profissional} parar de ler a tela.
...
mensagem nova por    a prévia morre. Não é erro: é {o-pessoa} falando. Volte ao
...
lista antes de perguntar é o que impede o {profissional} de esperar por nada. Use a
...
- **quem é o {profissional} na conversa colada**, quando nenhum remetente bate com
  `nome:` do `INDICE.md` — o risco é gravar a fala {do-pessoa} como promessa dele
- **se {o-pessoa} respondeu por fora**, quando o arquivo tem muitos `?` e o
  histórico é curto: “tem resposta dela depois de 5 de agosto que não está aqui?
  Se tiver, eu não retomo quem já voltou”
- **o que dizer**, quando o ângulo 4 depende de uma pergunta {do-pessoa} que
...
aparece com o motivo em uma linha. Sem isto o {profissional} não tem como saber que
faltou alguém — e descarte em silêncio é o que faz ele parar de confiar na
lista inteira.

```markdown
{retomar--saida-quem-ficou-de-fora}
```
...
```
{retomar--saida-cada-mensagem}
```

{retomar--a-procedencia}
...
incluída; ele zera quando {o-pessoa} responde. O número é para o {profissional} ler —
...
**O silêncio que ela mede é o silêncio do que está escrito.** Se {o-pessoa}
