...
que o {profissional} tem que fazer; o que ele está esperando alguém mandar é de
`/{plugin}:cobrar-o-que-falta`, que ordena por quem trava mais e escreve a
...
4.3  funil.md e as seis etapas       4.4  {o-item}      4.5  {o-pessoa}
...
`envio:`, o `{campo-horario-de-encontro}:` e o `canal padrão com
{pessoa}:`.
...
{profissional} quer ver andando é o dia dele, não a skill.
...
4  {pasta-itens}/_indice.md     estado e atualizado
...
promessa vence no dia seguinte ao prometido; parado é 7 dias; {hoje--os-cortes-do-oficio}
...
marcar {momento-encontro} sozinho.
...
```
prometido e não chegou   {o-pessoa} pode ter mandado no WhatsApp e ninguém
                         anotou. Cobrar documento que ele já mandou é o erro
                         que faz o {profissional} parar de confiar na lista
...
chegou é o erro que faz o {profissional} parar de confiar na lista; **mandar** essa
cobrança é o mesmo erro, com {o-pessoa} de testemunha.
...
```
{hoje--as-que-terminam-em-mensagem}
```
...
```
{hoje--a-escada-da-consequencia}
```

Empate dentro do mesmo degrau, nesta ordem: sobe quem tem **hora escrita**;
depois quem **espera há mais tempo**; e **dois itens do {mesmo-pessoa} não se
...
- **linha que outra skill acrescentou** — {hoje--quem-mais-escreve}
...
que o {profissional} está olhando. O painel não substitui a resposta; ele é onde
...
- **uma por execução.** A lista do dia não vira meia hora de trabalho sem
  ninguém olhando.
- **nunca `/{plugin}:{skill-conferir}`**, que é a exceção escrita do contrato:
  {o-que-o-conferir-lista}.
...
- **chamar a skill dona não pula a tela dela.** O automático entrega o trabalho,
  ...
  {profissional}, e o padrão dela é `pergunta sempre`.
...
Esta skill quase não pergunta — ela lê e resume, e o {profissional} abriu a sessão
...
- **a data em prosa**, e só quando o item cairia nos três primeiros: “o
  `## Combinado` da {exemplo-pessoa} diz sábado de manhã, sem data — é o
...
{pessoa}” não é item; “{exemplo-frase-acionavel}” é. O que ela não conseguiu apurar entra como `?` e vira linha em
...
nem arquivo de {pessoa} ou de {item}, nem `INDICE.md`, nem `_bruto/`. A razão é
...
- **as quatro seções ficam na página mesmo vazias**, com `- nada aqui hoje.`
  Sumir com a seção faz o {profissional} achar que a skill esqueceu.
- **`## Vence hoje` é o que tem de ser feito hoje, não o que expira hoje.** É
  por isso que a confirmação de uma {momento-encontro} de sábado entra nele na sexta — é a
...
- **o teto é 15 caixas no arquivo inteiro** (contrato, seção 9). Estourou: corte
  ...
  {itens} sem {exemplo-trabalho} ficaram fora do arquivo — estão na lista acima.” Item
...
aconteceu para `/{plugin}:organizar-carteira`. A lista de amanhã só é boa se a
