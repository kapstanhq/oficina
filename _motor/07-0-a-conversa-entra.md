...
ferramenta: o {profissional} exporta ou cola, e a skill lê os dois formatos que
...
| saber o que um áudio disse | perguntar ao {profissional} | vem transcrito, com a marca |
| guardar em `_bruto/` | igual nos dois | igual nos dois |

**O conector lê sempre, e manda uma por vez** — nunca em lote, e nunca sem o
{profissional} ter visto o texto e o nome de quem recebe. Como isso funciona está em
...
```
de pé e conectada          trabalhe, e não diga nada ao {profissional}
...
**Quem é o {profissional} na conversa:** é o remetente cujo nome bate com `nome:` do
...
{pessoa} como promessa do {profissional}.
...
```
[12/08/2026 14:41] {exemplo-pessoa-nome}: [áudio 0:42 · transcrição] amanhã de manhã eu consigo passar
```
...
Colagem sem carimbo de data e sem nome — um pedaço de conversa, {exemplos-de-texto-solto}, um e-mail encaminhado. Trate assim: o conteúdo é fato do que está
escrito, a data é a que o {profissional} disser (ou a de hoje, e a procedência diz
`← {profissional}, <hoje>`), e o autor não se adivinha.
...
1. **Grava o bruto primeiro**, em `_bruto/AAAA-MM-DD-<canal>-<apelido-curto>.md`, com
   ...
   porque, se algo der errado no meio, o material do {profissional} já está salvo.
2. **Extrai os fatos** para os arquivos donos — {pessoa} e {item} —, cada campo
