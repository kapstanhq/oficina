...
~/{pasta-base}/
  ...
  {pasta-itens}/
    _indice.md         uma linha por {item}
    {exemplo-item-arquivo}
  {pasta-pessoas}/
    _indice.md         uma linha por {pessoa}
    {exemplo-pessoa-arquivo}
...
[[se etapa-de:item]]
Quem anda no funil aqui é {o-item}: o `etapa:` mora no arquivo {do-item}, e o
`funil.md` tem uma linha por {item}. `{pasta-pessoas}/` pode estar vazia, e isso
não é {base} pela metade — {item} sem {pessoa} é o caso comum.
...
Neste documento, `~/{pasta-base}/…` é o modo curto de nomear o lugar, qualquer que
seja o transporte. É assim que se fala com o {profissional} no `local`; no `drive`,
diga “a pasta `{pasta-base}` do seu Drive”.

### Os dois transportes

```
local    uma pasta no computador do {profissional}
drive    uma pasta no Google Drive dele, pelo conector
```

Quem escolhe é o {profissional}, uma vez, no `/{plugin}:comecar`. **Nenhuma skill
troca o transporte, e nenhuma trabalha em dois ao mesmo tempo.**

**No `local`**, `~` é a pasta pessoal do {profissional}. No Windows é
...
a sessão abriu, e a sessão abre em qualquer lugar. Ao falar com o {profissional},
escreva `~/{pasta-base}/…`, que é curto e ele entende.

**No `drive` não existe caminho.** Pasta é um item com id, e arquivo é filho de
uma pasta. `/{pasta-base}` é o **nome** da pasta na raiz do Drive dele, não um
caminho que se entrega a uma ferramenta: achar `{pasta-itens}/_indice.md` é achar a
pasta `{pasta-base}`, achar a pasta `{pasta-itens}` dentro dela e procurar `_indice.md`
ali dentro. Guarde o id de cada pasta que abrir — reprocurar a mesma pasta a
cada operação é o que faz a execução demorar.

**A busca é sempre presa à pasta.** `_indice.md` existe duas vezes na {base},
em `{pasta-itens}/` e em `{pasta-pessoas}/`, e procurar pelo nome solto devolve os dois. Sem
saber de qual pasta veio, a skill grava a lista de {itens} por cima da lista de
{pessoas}.
...
### A linha `{pasta-base}:` do `INDICE.md`

É ela que diz o transporte e o lugar, e é a primeira coisa que a skill lê:

```
{pasta-base}: local · C:\Users\{exemplo-usuario}\{base}
{pasta-base}: drive · /{pasta-base}
```
...
onde o `INDICE.md` foi encontrado**, e a skill segue sem perguntar — {base}
...
`{pasta-base}: drive` implica `Google Drive: sim` em `## O que está conectado`: é o
mesmo conector, e a {base} que abriu é o teste.
...
|---|---|---|
| achar o `INDICE.md` | ler `<lugar>/INDICE.md` | procurar a pasta `{pasta-base}`, e `INDICE.md` dentro dela |
...
- **No `drive` não se atualiza arquivo — e isto não é limitação da skill, é da
  ...
  **Enquanto isso valer, a {base} no `drive` pode ser criada e não pode ser
...
|---|---|
| o conector do Drive não está ligado, ou perdeu a autorização | diz que a {base} está no Drive e o conector não respondeu, e para. Não procura pasta parecida no computador |
...
| a busca não achou a pasta pai | a {base} não está onde a linha `{pasta-base}:` diz. Mostra o lugar que tentou e para. **Não monta a {base} de novo** |
| a leitura falhou no meio | para. O que voltou pela metade se descarta e não se grava por cima; a skill diz qual arquivo era |
| a gravação falhou | diz qual arquivo não foi gravado, com o lugar, e o que ele deveria conter. O {profissional} precisa saber o que ficou de fora |
...
1. procura no `local`, em `~/{pasta-base}/INDICE.md`. Não há ferramenta de arquivo
   neste ambiente? Este degrau não existe: comece pelo 2
2. não achou, procura a pasta `{pasta-base}` no Drive e o `INDICE.md` dentro dela
3. leu: a linha `{pasta-base}:` diz o transporte, e **toda leitura e toda gravação
...
5. e há `INDICE.md` nos dois lugares? Isso é bifurcação, não detalhe: mostre as
   duas {bases}, com o lugar de cada uma, e pergunte qual fica. **Não funde as
   duas**
6. não há `INDICE.md` em lugar nenhum: a {base} não existe. A skill faz uma
   coisa só — diz isso em uma linha e manda rodar `/{plugin}:comecar`, que é
   quem pergunta o transporte. Vale em `local` e em `drive`

Não cria a {base} por conta própria, não trabalha sem ela, não improvisa em
...
[[fim]]
[[se etapa-de:item]]
| arquivo {do-item} | **dono** dos fatos {do-item}, inclusive `etapa:` | ele |
| arquivo {do-pessoa} | **dono** dos fatos {do-pessoa} — e ele não tem `etapa:` | ele |
[[fim]]
