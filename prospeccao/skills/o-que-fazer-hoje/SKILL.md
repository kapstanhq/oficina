...
description: >-
  Monta a lista do dia lendo a carteira inteira — reunião de hoje, resposta que
  chegou e não teve retorno, conta estudada e não abordada, proposta parada, quem prometeu
  documento e não mandou, quem está parado tempo demais e conta sem texto de abordagem.
  ...
  mais nenhum arquivo. Use de manhã, ou quando o prospector diz “o que eu faço
  hoje?”, “bom dia, o que tem pra hoje”, “por onde eu começo”, “me dá o dia”, “o
  que tá pegando fogo”, “quem eu abordo hoje?”, “fiquei três dias fora, o que
  ...
  sumiu, que é /prospeccao:retomar-contato, nem para arrumar a carteira, que é
  /prospeccao:organizar-carteira.
license: MIT
compatibility: >-
  Precisa da carteira do prospector, numa pasta do computador — ela lê a
  carteira inteira para ordenar o dia. Sem carteira, não funciona; diz isso em
  uma linha, manda rodar /prospeccao:comecar e não grava nada. A agenda é opcional — com o
...
que o prospector tem que fazer; o que ele está esperando alguém mandar é de
`/prospeccao:cobrar-o-que-falta`, que ordena por quem trava mais e escreve a
...
das doze skills do pack, e nada de formato se decide aqui; o inteiro está em
...
4.3  funil.md e as seis etapas       4.4  a conta      4.5  o contato
...
`04-2-hoje.md`, `04-3-funil.md`, `04-4-arquivo-de-conta.md`,
...
```
Não achei a sua carteira. Rode /prospeccao:comecar — ele monta com você, pergunta
onde ela fica e termina com uma conta e um contato de verdade lá dentro. Depois
...
`envio:`, o `horário de reunião que costumo oferecer:` e o `canal padrão com
contato:`.
...
`/prospeccao:estudar-conta`, e é por isso que o automático nunca a chama
...
prospector quer ver andando é o dia dele, não a skill.
...
```
1  funil.md               etapa, · desde e · próximo: de cada contato na carteira
2  contatos/_indice.md    último contato — dá a fila de quem abrir
3  cada arquivo de contato que a fila apontar
                          ## Combinado · ## Histórico · etapa: · canal:
4  contas/_indice.md     estado e atualizado
5  cada arquivo de conta que a fila cita, mais as contas em
                          a estudar ou a abordar
                          site: · estado: · ## O que a conta faz · ## O que abre a conversa
...
`## Falta saber` com o nome de `/prospeccao:organizar-carteira`. **Esta skill não
...
- **evento sem contato na carteira** entra na lista como está, com `?` no lugar
  do id, e vira uma linha em `## Falta saber`. Não se cria contato a partir de
  um título de evento.
- **hora divergente** entre a agenda e o `## Combinado` do contato: mostre as
...
7 dias; conta estudada e não abordada é 7 dias.

**O corte de 7 dias do “parado” é grosso de propósito.** Quem mede silêncio
direito é `/prospeccao:retomar-contato`, que tem prazo por etapa — aqui basta o
sinal, e a linha diz de quem é o assunto. Passou de 90 dias, é caso de
aposentar pela regra 3, e quem aposenta é `/prospeccao:organizar-carteira`: esta
...
prometido e não chegou   o contato pode ter mandado no WhatsApp e ninguém
                         ...
                         que faz o prospector parar de confiar na lista
...
que os arquivos não têm — e o conserto é `/prospeccao:organizar-carteira`, que é
...
chegou é o erro que faz o prospector parar de confiar na lista; **mandar** essa
...
```
respondeu, sem retorno    /prospeccao:escrever-abordagem   conversa viva, e é o
                          caso de menor risco que existe
proposta parada           /prospeccao:cobrar-o-que-falta   só depois de reler a
                          conversa, senão cobra o que já chegou
prometido e não chegou    /prospeccao:cobrar-o-que-falta   idem
parado tempo demais       /prospeccao:retomar-contato      com as travas dela:
                          7 dias de cadência, nunca a terceira, gancho novo
```

Os outros não terminam em mensagem, e o motivo é diferente em cada um: **reunião
de hoje** é entrar na chamada, e a confirmação da véspera já é o gatilho de
cima; **contato sem e-mail** não tem para onde mandar até alguém achar o
endereço; **conta sem estudo** não tem o que dizer ainda; e **conta estudada e
não abordada** já tem — e é a que mais dói ficar parada.
...
```
1  hora marcada hoje      reunião de hoje, compromisso de hoje na agenda
                          passou da hora, passou — não há como refazer o dia

2  janela que fecha       confirmação da véspera, proposta com prazo escrito
                          que termina hoje ou amanhã
...
3  quem respondeu         resposta sem retorno, pergunta em aberto
                          ...
                          não voltou, e é assim que uma conversa morre

4  papel que trava        proposta no jurídico, contato sem e-mail
                          o interesse já existe; o que segura é o processo

5  o que a semana come    quem não respondeu, retomada devida, conta
                          estudada e parada
                          um dia a mais não muda uma; a soma de trinta muda
                          o trimestre

6  só oportunidade        conta sem estudo, ficha pela metade
                          nenhuma conversa morre hoje por causa dela
```
...
Começo pelo primeiro?
  ...
  Responder o Paulo     /prospeccao:escrever-abordagem · leio a conversa e ela
                        te mostra o texto e o nome antes de sair qualquer coisa
  Confirmar a reunião   pronto agora, e a mensagem sai daqui com você vendo
...
que o prospector está olhando. O painel não substitui a resposta; ele é onde
...
decisão do prospector: não chame, e a linha do item diz o que falta. Quatro
travas, e elas não se negociam:

- **uma por execução.** A lista do dia não vira meia hora de trabalho sem
  ninguém olhando.
- **nunca `/prospeccao:estudar-conta`**, que é a exceção escrita do contrato:
  ela lista o que é público sobre a conta, com a procedência de cada fato, e
  quem decide se a conta serve é gente.
...
- **chamar a skill dona não pula a tela dela.** O automático entrega o trabalho,
  ...
  prospector, e o padrão dela é `pergunta sempre`.
...
Esta skill quase não pergunta — ela lê e resume, e o prospector abriu a sessão
...
- **a data em prosa**, e só quando o item cairia nos três primeiros: “o
  `## Combinado` da P-017 (Carla Menezes) diz sábado de manhã, sem data — é o
...
que não há nada vencendo, e que carteira sem conta nova há dias é assunto de
lista, não dia livre. Ofereça `/prospeccao:retomar-contato` ou
`/prospeccao:escrever-abordagem`, uma vez.
...
```markdown
Hoje, 19 de agosto — 6 itens

A reunião das 15h é a única coisa que passa da hora hoje. O Paulo vem antes das
cobranças porque ele perguntou o preço ontem e ainda não teve resposta — e
pergunta de preço sem retorno em 24 horas é a conversa esfriando na sua mão.

1  P-017 (Carla Menezes) · hoje, 15h
   Reunião sobre o fechamento do mês no E-071 (VetorBank, Porto Alegre). Ela ia
   dizer em 18 de agosto se o time de risco entra e não disse — mande uma linha
   antes.

2  P-024 (Paulo Tavares) · respondeu ontem, sem retorno
   Ele perguntou quanto custa. A resposta que fecha reunião não é o preço: é o
   tamanho do projeto e o próximo passo.
   → /prospeccao:escrever-abordagem · sai daqui
     Mando agora · Mudo o texto · Eu mesmo mando

3  P-031 (Sandra Lisboa) · proposta parada há 6 dias
   Ela ia levar ao jurídico em 13 de agosto. Cobre a etapa, não a decisão.
   → /prospeccao:cobrar-o-que-falta · sai daqui
     Mando agora · Mudo o texto · Eu mesmo mando

4  E-083 (Móveis Bertoldo, Bento Gonçalves) · estudada há 8 dias, sem abordagem
   Você já tem o gancho: trocaram de ERP em junho. Ele envelhece.
   → /prospeccao:escrever-abordagem — precisa do e-mail do P-019 (Rui Baptista)

5  P-041 (Otávio Prado) · abordado há 14 dias, sem resposta
   Precisa de gancho novo, não de “passando para saber”.
   → /prospeccao:retomar-contato · sai daqui
     Mando agora · Mudo o texto · Eu mesmo mando

6  E-052 (Clínica Sanare, Curitiba) · na carteira desde 11 de agosto, sem estudo
   Estudar antes de abordar — sem isso a mensagem sai genérica.
   → /prospeccao:estudar-conta — precisa do site
```
...
contato” não é item; “cobrar da P-031 (Sandra Lisboa) o retorno da
proposta do E-083 (Móveis Bertoldo, Bento Gonçalves)” é. O que ela não conseguiu apurar entra como `?` e vira linha em
...
aconteceu para /prospeccao:organizar-carteira e a lista de amanhã sai certa.
```
...
```markdown
## Guardei
- ~/carteira/hoje.md — reescrito, 6 caixas abertas e 2 do que você marcou nos últimos sete dias

## Falta saber
- que dia é a “semana que vem” combinada com a P-031 (Sandra Lisboa)
- de quem é a reunião das 11h na agenda — não achei contato com esse nome na carteira
- o contatos/_indice.md diz 11 de agosto para o P-019 (Rui Baptista) e o arquivo dele tem linha de 16; /prospeccao:organizar-carteira acerta a vista

## Decidi sozinho
- Respondi o P-024 (Paulo Tavares) pelo /prospeccao:escrever-abordagem, que era o primeiro item e tinha a conversa em _bruto/ — a mensagem está acima e não saiu: o seu envio: diz pergunta sempre, e ela está esperando você. Para não fazer isso, me diga e eu só listo.
- Li “semana que vem” como 26 de agosto, a segunda mais próxima. Se for outra, me diga e a cobrança sai da lista de hoje.
```

O `## Guardei` diz o lugar do jeito que o prospector reconhece, e o jeito muda com
...
nem arquivo de contato ou de conta, nem `INDICE.md`, nem `_bruto/`. A razão é
...
## Vence hoje
- [ ] Reunião 15h com P-017 (Carla Menezes), E-071 (VetorBank, Porto Alegre) — ela não confirmou desde 2026-08-16
...
- **as quatro seções ficam na página mesmo vazias**, com `- nada aqui hoje.`
  Sumir com a seção faz o prospector achar que a skill esqueceu.
- **`## Vence hoje` é o que tem de ser feito hoje, não o que expira hoje.** É
  por isso que a confirmação de uma reunião de sábado entra nele na sexta — é a
...
- **o teto é 15 caixas no arquivo inteiro** (contrato, seção 9). Estourou: corte
  ...
  contas sem texto de abordagem ficaram fora do arquivo — estão na lista acima.” Item
  cortado em silêncio é item perdido.

Se o prospector colar uma conversa no meio da execução, a ordem é a da seção 7 do
...
`/prospeccao:organizar-carteira` ou `/prospeccao:importar-a-conversa`, conforme o caso.
...
de busca: a lista do dia não precisa de dado novo de conta, e site que monta a
página por JavaScript devolveria nada de qualquer jeito. O CNPJ, o setor e a
cidade saem do arquivo, com a data que está lá.
...
quem tem prazo por etapa é `/prospeccao:retomar-contato`. Um contato pode aparecer
...
baixar preço, não promete prazo de jurídico, de compras ou de segurança da informação, não
