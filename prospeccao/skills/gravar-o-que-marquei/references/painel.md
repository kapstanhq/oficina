...
a ficha inteira de uma conta     um aviso curto
...
```
painel_inicio     { base: "<caminho inteiro da pasta da carteira>" }
                  → { painel: "<endereço>", diga: "..." }
```

O painel tem **casa**: uma página que fica de pé entre uma tarefa e outra, e
mostra o que a carteira já tem — o mapa do `INDICE.md`, a lista do dia, o funil
por etapa, cada arquivo com a procedência de cada campo, o que está conectado.
Ela **lê** a carteira e **não grava nada nela**: o que a pessoa faz ali volta
...
O prospector **decide quando quiser**, e não só quando você pergunta: na
página inicial, o contato que está numa etapa do funil tem dois botões —
passar para a etapa seguinte, e descartar. O clique NÃO grava na carteira: vai
...
Quem a grava SEM outro trabalho junto é `/prospeccao:gravar-o-que-marquei` — é a
skill que o botão "Gravar agora" do painel chama, porque a do dia lê a carteira
inteira para isso.
...
**Declare o `gesto` nas `decisoes`** de toda `lista` que julga contatos:
...
contato e o botão equivale a mudá-lo de etapa:
...
parou: a execução pode morrer esperando, e a carteira tem de saber onde.
...
**Completar informações**: chama `/prospeccao:completar-ficha` com o id. Quando
...
carteira pode ter o PRÓPRIO `painel.json`, na raiz dela, e o que ele diz vale
...
contato; `resumo`, títulos de seção; em `rotulos` e `proximo`, as skills
...
Na página de um contato, no cartão do início e na linha da tabela, o painel
mostra **o que vem agora**: um botão em destaque e os outros ao lado. A ordem
por etapa é do pack. Mas **você pode apontar o destaque de UM contato**, e o
lugar é a linha dele no `funil.md`, que você já escreve:

```
- E-071 (VetorBank, Porto Alegre) · desde 2026-09-08 · próximo: cobrar-o-que-falta — o documento que ficou de mandar
```

Quando o que vem depois de `próximo:` COMEÇA pelo nome de uma skill do pack —
`cobrar-o-que-falta`, ou `/prospeccao:cobrar-o-que-falta` —, ela vira o destaque daquele
contato, e o que vem depois do travessão é a nota que a pessoa lê debaixo do
...
`← prospector, no painel, AAAA-MM-DD`. `descartar` é a Regra 3 — aposentar,
...
carteira quem decide é você, pela regra do não apurado.
...
compacta quando a `linha` basta. Com a carteira aberta, o item cujo `id` tem
...
para a carteira pelas regras de sempre, com a origem `← prospector, no
painel`.
...
- **Quando o prospector pediu para não usar.** É a ferramenta dele.
