...
description: >-
  Varre a busca e lista o que está parado esperando OUTRA PESSOA — quem
  ...
  texto e o destinatário na tela antes. Use quando o candidato disser — o
  ...
  /vagas:retomar-contato, nem para a lista do dia, que é
  /vagas:o-que-fazer-hoje.
license: MIT
compatibility: >-
  Precisa da busca; sem ela diz isso em uma linha e manda rodar
  /vagas:comecar. Sem o conector ela escreve as cobranças para copiar e quem
  manda é o candidato — o trabalho sai inteiro do mesmo jeito. Com o
...
Isso é o oposto do resto do pack. `/vagas:o-que-fazer-hoje` lista o que o
candidato tem que fazer; esta lista o que ele **não pode fazer** porque está
...
- **Não é retomada.** Quem sumiu do funil sem dever nada é
  `/vagas:retomar-contato`. Aqui a pessoa **deve uma coisa concreta**, e a
  diferença muda a mensagem inteira: uma traz novidade, esta traz o pedido.

## 2 · Antes de tudo

1. `~/busca/INDICE.md`. Não existe: uma linha e `/vagas:comecar`.
2. A linha `modo:`.
...
coisa**. O candidato escolhe quem recebe.
...
2. **Os `?` com dono** nos arquivos de `vagas/` e `contatos/`. Um
...
Uma pendência é `o retorno da candidatura da V-027 (Head de Produto, Aurora Saúde), enviada pelo site em 2026-09-02` — quem, o quê, desde quando.
...
- **Cobrado duas vezes sem resposta.** Passa a ser conversa do candidato,
...
A ordem se explica em uma linha por item — o candidato precisa saber por que
...
gravar depois, um envio que falha no meio deixa a busca dizendo que cobrou
...
**Quem deve pode não ter arquivo.** Aqui uma vaga anda com o campo
`contato:` vazio, e a pendência é de uma empresa, de um
setor, de um endereço genérico. Nesse caso a tentativa vai para o
`## Histórico` da vaga de que a pendência trata — com a mesma linha —, e é
lá que o Passo 2 a procura na próxima rodada. Não crie contato só para ter
onde gravar: contato nasce de nome e canal de verdade, não de uma cobrança.
...
candidato copia e cola. Nada de moldura desenhada.
...
**1 · o retorno da candidatura da V-027 (Head de Produto, Aurora Saúde), enviada pelo site em 2026-09-02**
há 27 dias · sem ela a candidatura não anda
```
...
## Fora da fila
- P-005 (Helena Prates) · cobrado em 2026-09-07, há 2 dias — espere até o dia 14
...
**Ela não cobra dinheiro.** Rescisão, reembolso e pagamento atrasado não são
...
quem escreve é o candidato, que sabe o que foi combinado.

**Ela não manda para quem nunca escreveu.** Contrato §7.1: a primeira mensagem
para um número novo sai do celular do candidato. Aqui isso quase nunca
...
conversa e manda `/vagas:organizar-busca`, que é quem extrai fato com
