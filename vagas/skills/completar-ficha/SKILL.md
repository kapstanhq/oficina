...
description: >-
  Procura o que falta em uma vaga da busca — os campos do cabeçalho que estão
  em `?` — nas fontes que a busca alcança: a origem guardada em `_bruto/`, o
  ...
  candidato disser — completa essa ficha · procura o que falta · o que ela
  ...
  chamar. Não é para julgar se serve, que é /vagas:triar-vagas, nem
  para cobrar alguém, que é /vagas:cobrar-o-que-falta.
license: MIT
compatibility: >-
  Precisa da busca e do id de um registro dela. O que ela alcança depende do
...
ele entrou. Alguns decidem se ele serve ao candidato, e ele abre a ficha,
...
candidato confirmar — não é fato, e não se completa aqui. Se ele e as
...
4.4  o arquivo da vaga             4.7  o bruto
4.6  os dois índices                 10   como uma skill começa e termina
```

Os arquivos: `02-0-id-e-apelido.md`, `03-0-as-tres-regras.md`,
`04-4-arquivo-de-vaga.md`, `04-6-os-dois-indices.md`, `04-7-o-bruto.md` e `10-0-comeca-e-termina.md`. E
...
dono dele. Não chegou: pergunte qual, em uma linha. Não percorra a busca
...
Conta também o campo que o `04-4-arquivo-de-vaga.md` tem e o arquivo não: registro
...
`04-4-arquivo-de-vaga.md` escreve um caminho próprio**: esse segue o caminho de
...
- se a fonte diz que o registro **saiu do ar** ou fechou, isso é fato: o campo
  ...
  `/vagas:organizar-busca` — ela não aposenta
...
- a linha do `funil.md` e o `_indice.md` que repetem um campo que mudou mudam
  juntos (Regra 1: o arquivo da vaga é o dono, as vistas acompanham)
...
## Guardei
- ~/busca/<arquivo da vaga> — os campos e o histórico
- ~/busca/_bruto/<arquivo> — a página que li

## Falta saber
- <o campo que só o candidato ou a organização respondem, e a quem perguntar>
````
...
linha no fecho, apontando `/vagas:triar-vagas`.
...
candidato conferir cada fonte antes de o campo virar verdade na busca.
