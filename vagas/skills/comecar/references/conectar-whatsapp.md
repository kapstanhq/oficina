...
Não são letra miúda: são o que faz o candidato decidir com o que ele tem, e o
...
`whatsapp-reader estado` sabe dizer. Conte isso ao candidato: é mais barato
do que ele descobrir sozinho, e é o motivo do degrau seguinte.

### 4.5 · Vivo depois de desligar o computador

```
verificar   o candidato reinicia a máquina e `whatsapp-reader estado` ainda
...
elevação, e o candidato pode não ter. A saída não precisa de nada disso: um
...
Uma coisa que o agendador **não** resolve, e o candidato tem que ouvir: de
...
transcrita, e o que o contato combinou falando entra na busca como entra o
que ele escreveu.
...
agir        mandar UMA mensagem para o número do próprio candidato
...
da ponte, que a busca não alcança:

```
whatsapp-reader nao-contatar 5511900000012 "pediu em 12/08"
```

Diga ao candidato que ele mesmo pode rodá-lo, e que `--tirar` desfaz. Pedido
de silêncio que fica só na busca continua valendo para as skills, mas não
...
O que já entrou na busca fica: é dele, e tem procedência.
