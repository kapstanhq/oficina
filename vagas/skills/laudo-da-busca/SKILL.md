...
name: laudo-da-busca
description: >-
  A régua barata da busca: lê tudo e não escreve nada. Diz o que está sem
  procedência, o que está com procedência VENCIDA, que vaga ou contato ficou
  ...
  por pasta. Use antes de mexer na busca e depois de escrever nela, e quando
  o candidato disser — a busca está confiável? · dá uma conferida no que
  eu tenho aí · o que está faltando na minha busca · tem coisa velha aqui? ·
  o que está desatualizado · essa informação ainda está certa? · antes de eu
  mandar isso, confere
  · será que perdi alguma coisa · minha busca está uma bagunça, o que tem de
  errado. Também depois de importar planilha ou conversa em volume. Ela não
  arruma nada: quem arruma é /vagas:organizar-busca, e quem cobra o que
  falta de outra pessoa é /vagas:cobrar-o-que-falta.
license: MIT
compatibility: >-
  Precisa da busca e só de leitura — não escreve, não move e não apaga, em
  nenhum modo. Sem busca ela diz isso em uma linha e manda rodar
  /vagas:comecar. Funciona igual no computador e no Drive; no Drive ela lê
...
Ela **lê a busca inteira e não escreve uma linha**. O que ela devolve é um
...
horas importam pela mesma razão: uma busca pouco aberta desatualiza, e
busca desatualizada faz toda skill mentir com confiança. O `?` honesto vira
frase afirmativa, a vaga aberta em março vira vaga aberta hoje, e o candidato
...
não aposenta ninguém. Quem faz isso é `/vagas:organizar-busca`, e a
...
1. Leia `~/busca/INDICE.md`. Não existe: uma linha dizendo que não há
   busca para conferir, e `/vagas:comecar`. Não invente busca vazia.
...
5. Monte o inventário sem abrir tudo: os dois `_indice.md`, o `funil.md`, o
   `hoje.md` e a lista de nomes de `vagas/`, `contatos/` e
...
marca de origem é o achado mais grave da busca, porque ele é indistinguível
de um campo apurado — e é exatamente o que a próxima skill vai afirmar para
alguém de fora.

Procure, em `vagas/` e `contatos/`, a linha de campo que tem
...
```
se a vaga está aberta                   7 dias   vaga boa fecha em duas semanas, e candidatar-se à que fechou é tempo jogado fora
o que a vaga pede, o regime e a faixa   30 dias   a empresa republica com outro texto, e o anúncio antigo engana
quem é o contato, e o cargo dele        90 dias   recrutador troca de empresa mais rápido que a vaga fecha
e-mail e telefone do contato           180 dias   a pessoa continua lá, o ramal não
o que o contato disse do processo       30 dias   prazo de contratação escorrega com o trimestre
```

Regra geral, para o que não estiver na lista: **fato que depende de terceiro
vence; fato que depende do candidato não vence.** O nome de quem decide não
envelhece; o estado de uma vaga, a faixa e o prazo de qualquer coisa, sim.

O achado é `arquivo · campo · apurado em <data> · há N dias`, e o conserto é
sempre perguntar de novo — nunca deduzir.

### 4.3 · Que vaga ou contato ficou órfão
...
- **id sem apelido** — o contrato pede `V-019 (Gerente de Produto Sênior, Trilho Logística)`, nunca o id sozinho.
  Onde o apelido faltar, o candidato não sabe do que se está falando.

E uma quinta, que só existe porque aqui a etapa é da vaga:

- **etapa sem dono** — `etapa:` num arquivo do contato. O `funil.md` sai dos
  arquivos da vaga, e aquele campo ninguém lê: ele existe e não vale. O
  achado nomeia o arquivo; quem decide o que fazer com a linha é o candidato.

**O que NÃO é órfão:** vaga com `etapa:` e `contato` vazio.
Vaga sem contato é o caso comum, e listá-lo enche o laudo de linha que
ninguém vai consertar. Órfão é o contrário — o campo nomeia um id, e o id não
tem arquivo em `contatos/`: é a primeira forma, e entra como ela.
...
O `?` é a lista de trabalho da busca, e ela envelhece em silêncio. Ordene por
...
Um `?` que espera há mais tempo que o 45 dias não é mais uma
...
Se a busca estiver ilegível — pasta sem permissão, arquivo corrompido, Drive
...
coisa, e é o ponto: quem lê precisa saber que a busca **não mudou**.
...
dele. Não é a lista dos `?` da busca — essa é a seção 4.5, e ela é o
...
qual é o novo. Julgar a vaga é do candidato, e as skills que ajudam nisso
têm nome próprio.

**Ela não abre link e não sai da busca.** O `?` que um link resolveria é
achado, não trabalho — varrer a busca abrindo trinta páginas transforma uma
