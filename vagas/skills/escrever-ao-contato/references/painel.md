...
a ficha inteira de uma vaga     um aviso curto
...
**Ele não escreve na busca.** Nem um byte. O painel devolve a INTENÇÃO —
“clicou em tal coisa, no item tal” — e quem grava é você, com as regras de
sempre: procedência, histórico, id com apelido, os tetos.

Isso não é detalhe de implementação: se o painel gravasse, a busca teria
...
```
painel_inicio     { base: "<caminho inteiro da pasta da busca>" }
                  → { painel: "<endereço>", diga: "..." }
```

O painel tem **casa**: uma página que fica de pé entre uma tarefa e outra, e
mostra o que a busca já tem — o mapa do `INDICE.md`, a lista do dia, o funil
por etapa, cada arquivo com a procedência de cada campo, o que está conectado.
Ela **lê** a busca e **não grava nada nela**: o que a pessoa faz ali volta
como intenção, e quem grava continua sendo você.

**Chame `painel_inicio` uma vez, no começo de toda execução que tem a busca**,
com o caminho que está na linha `busca:` do `INDICE.md`. O endereço é
...
O candidato **decide quando quiser**, e não só quando você pergunta: na
página inicial, o vaga que está numa etapa do funil tem dois botões —
passar para a etapa seguinte, e descartar. O clique NÃO grava na busca: vai
...
Quem a grava SEM outro trabalho junto é `/vagas:gravar-o-que-marquei` — é a
skill que o botão "Gravar agora" do painel chama, porque a do dia lê a busca
inteira para isso.
...
vaga e o botão equivale a mudá-lo de etapa:
...
parou: a execução pode morrer esperando, e a busca tem de saber onde.
...
busca pode ter o PRÓPRIO `painel.json`, na raiz dela, e o que ele diz vale
...
vaga; `resumo`, títulos de seção; em `rotulos` e `proximo`, as skills
...
Na página de uma vaga, no cartão do início e na linha da tabela, o painel
mostra **o que vem agora**: um botão em destaque e os outros ao lado. A ordem
por etapa é do pack. Mas **você pode apontar o destaque de UM vaga**, e o
lugar é a linha dele no `funil.md`, que você já escreve:

```
- V-019 (Gerente de Produto Sênior, Trilho Logística) · desde 2026-09-08 · próximo: cobrar-o-que-falta — o documento que ficou de mandar
```

Quando o que vem depois de `próximo:` COMEÇA pelo nome de uma skill do pack —
`cobrar-o-que-falta`, ou `/vagas:cobrar-o-que-falta` —, ela vira o destaque daquele
vaga, e o que vem depois do travessão é a nota que a pessoa lê debaixo do
...
`← candidato, no painel, AAAA-MM-DD`. `descartar` é a Regra 3 — aposentar,
...
busca quem decide é você, pela regra do não apurado.
...
compacta quando a `linha` basta. Com a busca aberta, o item cujo `id` tem
...
para a busca pelas regras de sempre, com a origem `← candidato, no
painel`.
...
- **Quando o candidato pediu para não usar.** É a ferramenta dele.
