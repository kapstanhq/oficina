...
description: >-
  Grava na busca as decisões que o candidato marcou no painel sem estar
  numa conversa — vaga que passa para a etapa seguinte do funil e
  vaga que sai, com o motivo. Lê a fila do painel, grava cada decisão pelas
  ...
  o candidato disser — grava o que eu marquei no painel · grava as minhas
  marcações · passa pra busca o que eu decidi no painel · salva o que eu
  marquei · atualiza o que eu marquei — ou quando o botão do painel a chamar.
  Não é para a lista do dia, que é /vagas:o-que-fazer-hoje, nem para arrumar
  a busca inteira, que é /vagas:organizar-busca.
license: MIT
compatibility: >-
  Precisa da busca e do painel do pack — a fila mora nele, e chega por
  `painel_inicio` e `painel_fila`. Sem o painel nesta sessão, diz isso em uma
  linha e não grava nada. Sem busca, manda rodar /vagas:comecar.
...
Na página inicial do painel o candidato decide quando quiser: a vaga
que está numa etapa do funil tem dois botões — passar para a etapa seguinte, e
descartar. O clique não grava na busca: vai para uma fila do painel, e fica lá
até alguém gravar.

**Esta skill grava a fila, e só a fila.** Ela existe para isso custar pouco:
toda skill grava a fila ao começar, mas a do dia lê a busca inteira para
...
4.4  o arquivo da vaga             4.5  o arquivo do contato
4.6  os dois índices                 10   como uma skill começa e termina
```

Os arquivos: `03-0-as-tres-regras.md`, `04-1-indice.md`, `04-3-funil.md`,
`04-4-arquivo-de-vaga.md`, `04-5-arquivo-de-contato.md`,
`04-6-os-dois-indices.md` e `10-0-comeca-e-termina.md`. E
`references/painel.md`, a seção "A fila de decisões".

Depois, o `INDICE.md` da busca, pela linha `busca:`. Sem busca: uma
linha dizendo isso, `/vagas:comecar`, e nada gravado.

**A fila vem do painel.** Chame `painel_inicio` com o caminho da busca: ele
...
As decisões já são do candidato: ele as tomou no painel, com o botão. Por
...
Decisão com `envelheceu: true` é de uma vaga que já não está na etapa `de`:
...
do painel já avisou o candidato.

### Passo 3 · As passagens de etapa

Para cada `etapa`, no arquivo da vaga: a linha `etapa:` passa a `para`,
com a procedência `← candidato, no painel, AAAA-MM-DD`, e o `## Histórico`
...
arquivo, o arquivo para `arquivo-morto/vagas/`, a linha do
...
o arquivo da vaga já é o dono do fato.

### Passo 5b · As respostas que chegaram atrasadas

`painel_inicio` pode trazer `respostas`: telas que o candidato respondeu
...
## Guardei
- ~/busca/vagas/<arquivo> — a etapa e o histórico
- ~/busca/arquivo-morto/vagas/<arquivo> — aposentado
- ~/busca/funil.md, _indice.md e INDICE.md — as vistas e os contadores
...
conversa, e acontece no painel ou em `/vagas:triar-vagas`.

**Ela não refaz a lista do dia.** Tira do `hoje.md` o que foi aposentado, e só.
A lista nova é `/vagas:o-que-fazer-hoje`.
